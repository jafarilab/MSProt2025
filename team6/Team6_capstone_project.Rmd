...
psm_df
#Converted into dataframe because it is just easier to check in environment and easier to work on for me; for exploring and visualization only. we will do filtering in actual object psms in later steps
...
```{r}
#Apply filtering based on FDR or identification score ?
psms <- psms %>% filterPsmRank() #keeping only those with rank 1 
...
dim(psm_df)
nrow(psm_df)
length(unique(psm_df$pepseq)) 
length(unique(psm_df$accession))
```
...
```{r}
sessioninfo::session_info() #versions
```
